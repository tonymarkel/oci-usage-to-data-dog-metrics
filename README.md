## OCI USAGE TO DATA DOG METRICS - Creates Oracle Cloud Infrastructure Usage and Cost in a standard DataDog Metric format
...
Notable differences: 
* Only works with the -report PRODUCT flag
* Defaults to yesterday's usage, use the -ds and -de flags
To gather historical data, it is recommended to run in a bash loop, incrementing the start and end date so that one day's usage is printed for each day.
